...
let appsDescriptions = ["Edutoets Admin makes it easy for teachers to create tests and make tests available for students. The admin app gives insight in the results after the test has been made. Students can make the tests using the Edutoets Afname app.", "NewLinQ is an app that allows you to connect and chat with other users. The app is invite-only and users can make a profile to show their specialities and tell other users about themselves. The app contains a gamification effect where users can accumulate activity points.", "Keep track of your billiard score with the Carambole Counter app. Easily enter the amount of caramboles you scored during each round and generate a PDF file with an overview of the game when finished. The history of played games and earned points is saved.", "With the TUDirect app, students of the Delft University of Technology can easily get insight in their grades, courses and schedule. This app makes use of the TU Delft API that provides open data.", "Camuse is a company in Bergen op Zoom that rents camera material. Users can use the app to learn more and inquire about rented products."]
let appsScreenshots = [
    ["edutoetsscreenshot1", "edutoetsscreenshot3", "edutoetsscreenshot2"],
    ["newlinqscreenshot1", "newlinqscreenshot2", "newlinqscreenshot3"],
    ["carambolecounterscreenshot1", "carambolecounterscreenshot2", "carambolecounterscreenshot3"],
    ["tudirectscreenshot1", "tudirectscreenshot2", "tudirectscreenshot3"],
    ["camusescreenshot1", "camusescreenshot2", "camusescreenshot3"]
]
...
class AppsViewController: PageViewController
{
    ...
    func appsGraphAnimationFinished(notification: NSNotification)
    {
        ...
        UIView.animateWithDuration(0.4, animations: { () -> Void in
            self.appDescriptionView!.transform = CGAffineTransformMakeScale(1, 1)
        }) { (b: Bool) -> Void in
                // ...
        }
    }
